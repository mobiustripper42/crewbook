...
### DEC-114: Xola plugin auth — one user key, seller scoping via URL
**Decision:** BrewBoat authenticates to Xola as a single App Store plugin user — one `XOLA_PLUGIN_KEY` (the `user.apiKey` returned by `POST /api/plugins`) used for every data call across every seller that has installed the plugin. Multi-tenancy is expressed in the URL path (`/api/sellers/{seller_id}/…`), not in the credential. All requests carry both `X-API-Key: <plugin key>` and `X-API-Version: 2021-03-10`.
**Why:** Xola's App Store model is the documented integration path (DEC-111); plugins authenticate as themselves and inherit access via the seller-side install flow + `installation.create` webhook. A per-seller credential model (OAuth-style token-per-tenant) is not how Xola is designed, and rolling our own would mean reverse-engineering an undocumented surface. The version header is not optional — Xola pins behavior to it, and omitting it can return stale shapes or 4xx depending on endpoint.
**Implications:**
- The seller ID is configuration, not a path the app discovers at runtime. `XOLA_SELLER_ID` env var carries `69dfbe5744e51dad92085ae5` for sandbox; production gets a separate value at Phase 6.4. When V2 onboards a second seller, the env-var pattern becomes inadequate — at that point we move seller IDs to a configuration table keyed off the `installation.create` webhook payload. Out of scope for V1.
- 403/404 on `/api/guides`, `/api/users`, `/api/sellers` (un-prefixed) is **not** a permissions error — those endpoints don't exist. Diagnosing as "scope problem" leads to wrong tickets to Xola support. The Phase 1.2 → 1.3 transition burned a session on this misdiagnosis before the App Store docs clarified the routing model.
- Permission changes in the developer console **un-approve** the plugin until Xola re-approves. Treat permission edits as a deploy-blocking operation.
**Status:** Decided 2026-05-21 (Session 8). Codified in [`docs/XOLA_INTEGRATION.md`](./XOLA_INTEGRATION.md) — the canonical operational reference.
**Revisit:** Phase 6 (V2 multi-seller) — replace the single `XOLA_SELLER_ID` env var with an installations table populated by the webhook. At that point, also revisit whether the plugin permissions set is still the right least-privilege list.

### DEC-115: Xola order mirror policy — sync confirmed + cancelled, status preserved in raw
**Decision:** The `xola_orders` mirror table stores every order whose `items.status` is in the confirmed family (`200`, `201`, `202`, `203`) **or** explicitly cancelled (`700`). Pending and hold statuses (`100`, `101`, `103`) are excluded at fetch time. The full Xola order payload — including all items and their per-item status — lives in the `raw` jsonb column; no top-level status field is denormalized in the Phase 1.4 schema. Downstream code (Phase 1.6 mapping, Phase 2 shift agent) reads status out of `raw->'items'`.
**Why:** This is the V1 answer to "what happens when a customer cancels?" — three patterns were considered:
- (A) Mirror confirmed only, never delete. Stale phantom rows; schedule board lies after a cancellation.
- (B) Mirror confirmed + cancelled, store status. Mirror tracks state transitions truthfully; one extra row per cancellation is cheap.
- (C) Mirror confirmed only, delete-missing-on-sync. A single failed sync (network, Xola 500s) wipes the table; dangerous.
(B) was picked. Pending (100s) is excluded because abandoned-cart orders carry no scheduling signal and add bandwidth. The 5 statuses we capture are the deterministic set: anything that did or did not happen on the water.
**Multi-item gotcha:** since 2017-09-28, a Xola order can carry multiple items on different dates. `items.status` is per-item. A single "order status" doesn't exist for multi-item orders. The Phase 1.4 schema sidesteps this by not denormalizing status at all — it stays in `raw`. Phase 1.6 (orders → events mapping) is where items get unpacked into the event grain; that's the right layer to handle mixed-status orders. BrewBoat is one-trip-per-item in practice, so the multi-item case is rare but documented.
**Stale rows:** the mirror is not garbage-collected in V1. A confirmed order whose status flips to cancelled between syncs is updated in place (upsert by id). A row that disappears entirely from Xola (deleted via Xola admin tooling, exceedingly rare) becomes a permanent stale row. Manual cleanup via SQL is acceptable; a reconcile job is V2.
**Forward direction (V2 / Phase 6+):** Xola fires order lifecycle webhooks (`order.create`, `order.update`, `order.cancel`). When V1 stabilizes, subscribe to those webhooks so the mirror updates in real time; the admin "Pull Week" button becomes a reconciliation backstop instead of the primary refresh path. The webhook receiver plumbing also unlocks `installation.create` for V2 multi-seller (per DEC-114). Not in V1 scope because (a) public endpoint + signature verification + idempotency + local-dev tunneling is a 2–3pt task on its own, and (b) Drew's 5–15 orders/week make on-demand-pull latency acceptable.
**Status:** Decided 2026-05-21 (Session 8, Task 2). Issue #25 originally scoped "confirmed only" — broadened to confirmed + cancelled here based on the option-B analysis. Issue scope updated accordingly.
**Revisit:** Phase 6 (V2). Switch primary refresh to webhooks; reduce Pull Week to a backstop; reconsider whether pending statuses (100s) become useful for any preview workflow.

### DEC-116: Profile identity decoupled from `auth.users`
**Decision:** `profiles.id` is a standalone UUID (default `gen_random_uuid()`); the link to Supabase Auth lives in a new nullable `auth_user_id uuid references auth.users(id) on delete set null` column. The Xola cross-reference lives in a new nullable `xola_guide_id text references xola_guides(id) on delete set null` column (one-to-one for V1 single-seller). The `public.is_admin()` helper now matches `auth_user_id = auth.uid()` instead of `id = auth.uid()`.
**Why:** Phases 2–4 (shift gen, staff assignment, write-back) need crew profile rows to exist *before* the crew member has logged in for the first time — magic-link auth (DEC-104) isn't built until Phase 5.1. Under the old model (`profiles.id references auth.users(id) on delete cascade`), creating a profile required first creating an `auth.users` row, which either forced operational toil (Drew creating dummy users in the Supabase dashboard for each crew member) or hidden complexity (the admin UI calling the Supabase Admin API to create fake auth users on demand). Decoupling lets Drew populate the staff cross-reference table (Phase 1.8) and run the assignment agent (Phase 3) against profiles that don't yet have auth — Phase 5.1 fills in `auth_user_id` when each crew member first follows their magic link.
**Implications:**
- `is_admin()` MUST be rewritten as part of this migration. A wrong rewrite locks every admin out of every admin-gated RLS policy. Pinned by pgTAP.
- Existing FKs from `assignments.profile_id` and any future table to `profiles.id` keep working unchanged — the migration doesn't rewrite `profiles.id` for existing rows; only the FK constraint on `profiles.id → auth.users.id` is dropped.
- Phase 5.1 magic-link flow sets `auth_user_id` on first sign-in (lookup by `xola_guides.email` → `profile.xola_guide_id` chain, or admin pre-link). Until 5.1, all profiles have `auth_user_id = null` except Drew's seeded admin row.
- The xref grain is a column, not a join table — V1 is single-seller per DEC-114, so one Xola guide maps to one profile. V2 multi-seller (one person working two breweries) revisits with a join table; out of scope.
- Email-null guides (sandbox shows them; prod will too) are linked manually via the Phase 1.8 admin picker. Auto-match suggests email-equal first, then name fuzzy-match; admin always confirms.
**Status:** Decided 2026-05-27 (Session 9). Migration: `supabase/migrations/20260527021619_profile_identity.sql`.
**Revisit:** Phase 5.1 (magic-link auth) — `auth_user_id` population path must be wired. V2 multi-seller (DEC-114 revisit) — promote `xola_guide_id` column to a join table.
