# BrewBoat Crew Scheduler — Claude Code Project Context

## What We're Building
BrewBoat is a crew-scheduling tool for a small brewery-boat operation. Reservations originate in Xola (DEC-102). BrewBoat reads them, runs an AI agent to generate weekly shifts (DEC-103), runs a second agent to assign captains/mates/shore staff against staff availability, lets the admin review and adjust on a schedule board, then writes assignments back to Xola as guide assignments. Phase 5 adds a mobile-first staff self-select view — crew claims open shifts from their phone at the dock.

Roles:
- **Admin** — manages staff roster, reviews generated shifts, approves the week, pushes to Xola. Drew is the operational admin.
- **Captain** — runs a brewboat or captained duffy (license required, qualified per boat).
- **Mate** — assists the captain on a brewboat (no captain's license).
...
- **Notifications:** Resend (email) — magic links for staff auth (DEC-104), shift-posted notifications (Phase 5.7). No SMS / no Twilio.
- **Hosting:** Vercel (frontend), Supabase Cloud (database)
- **Testing:** pgTAP (RLS), Playwright (integration), axe-core (accessibility) — coverage baked into task estimates (DEC-109)
...
Sketch (refine in Phase 0.3 when the initial migration lands):

```
xola_orders (mirrored)        → xola_events (mirrored, time slots)
                                       ↓
                                product_type_lookup (DEC-108: 19 names → 3 types)
                                       ↓
                                   shifts (generated by shift agent — Phase 2)
                                       ↓
                                 assignments (staff × shift, DEC-107 role-aware)
                                       ↑
profiles (staff)  ─ role flags ─┤      │
                  ─ boat quals ─┤      │
                                       │
                          availability (staff × week × time range)
                                       │
                       scheduling_runs (audit: agent inputs, outputs, version)
```

Key relationships:
- `shifts` derive from `xola_events`, *not* directly from `xola_orders`. Multiple orders can fill one event/time slot.
- `assignments.role` distinguishes captain / mate / shore — a `brewboat` shift requires both captain and mate; a captained duffy requires captain; a duffy rental requires shore.
- `scheduling_runs` records every agent call for replay/debugging — input shifts + output assignments + agent prompt version.
